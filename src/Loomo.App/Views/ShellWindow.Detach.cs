...
public partial class ShellWindow {
    ...
    private DetachedItem? RestoreDetachedItem(DetachedItemSnapshot snapshot) {
        ...
        if (kind == DetachKind.EditorSupportMirror && !string.IsNullOrWhiteSpace(snapshot.FilePath)) {
            ...
            var view = new DetachedEditorSupportView(_editorSupports, _editorSupport.Pipeline, _editorSupport.WebView.ViewFactory, _settings, _workspace.RootPath, source.Control);
    ...
    private void OnDetachEditorSupport(object sender, RoutedEventArgs e) {
        ...
        if (source is null)
            return;
        var view = new DetachedEditorSupportView(_editorSupports, _editorSupport.Pipeline, _editorSupport.WebView.ViewFactory, _settings, _workspace.RootPath, source);
