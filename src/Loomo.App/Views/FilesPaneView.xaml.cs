...
/// <summary>ファイル一覧（エクスプローラ）ペインの View。1フォルダーぶんを詳細リストで並べ、
/// 並べ替え・絞り込み・複数選択・ドラッグ＆ドロップを担う。
///
...
public partial class FilesPaneView : UserControl
{
    ...
    // Loomo 自身が発生源のドラッグ中フラグ（内部ドラッグは移動、外部からはコピーを既定にする）。
    ...
    /// <summary>ペインがフォーカスされたときの入り口（<c>ShellWindow.FocusPane</c>）。</summary>
    ...
    {
        if (e.PropertyName != nameof(FilesPaneViewModel.PendingSelection) || Vm?.PendingSelection is not { } path)
            return;
    ...
    private void OnListPreviewKeyDown(object sender, KeyEventArgs e)
    {
        ...
        switch (e.Key)
        {
            ...
            case Key.Back:
                ...
                break;
            case Key.F2:
                RenameEntry(EntryList.SelectedItem as FileEntryViewModel);
                e.Handled = true;
                break;
            case Key.Delete:
    ...
    /// <summary>選択の中身に合わせて項目を出し分ける。Tag の意味は
    /// <c>Selection</c>＝1件以上選択、<c>Single</c>＝ちょうど1件、<c>FileOnly</c>／<c>DirOnly</c>＝
    /// 1件かつファイル／フォルダー、<c>Html</c>＝1件かつ HTML、<c>CompareTwo</c>＝ファイルちょうど2件。</summary>
    private void OnContextMenuOpened(object sender, RoutedEventArgs e)
    {
        ...
        // 「開く」は選択が1件のときだけ意味がある（Tag が無いので個別に）。
        if (menu.Items.OfType<MenuItem>().FirstOrDefault() is { } openItem)
            openItem.Visibility = single is not null ? Visibility.Visible : Visibility.Collapsed;
    ...
    // ===== ドラッグ＆ドロップ =====
    // 一覧内のフォルダーへのドロップで移動、外部（エクスプローラー等）からのドロップでコピー。
