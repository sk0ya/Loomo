...
internal sealed class EditorSupportController
{
    ...
    /// 呼ばれるのは <c>EditorSupportRenderEngine.Emit</c>（キャンセル確認の直後）だけ。
    ...
    public int? CurrentOutlineLine(EditorTab? source, string? filePath, int line, int column)
    {
        if (!OutlineMatches(source, filePath) || OutlineRoots is not { } roots)
            return null;
        return CodeOutline.FindEnclosing(roots, line, column) is { } member
            ? member.Line0 + 1
            : 0;
    }
    ...
    public bool TryChangeSource(
        EditorTab source,
        bool force,
        EventHandler viewportScrolled,
        EventHandler<CaretInfo> caretMoved)
    {
        ...
        if (Source is { IsRealized: true } previous)
        {
            previous.Control.ViewportScrolled -= viewportScrolled;
            previous.Control.CaretMoved -= caretMoved;
        }
        Source = source;
        if (!IsNavigating)
        ...
        {
            source.Control.ViewportScrolled += viewportScrolled;
            source.Control.CaretMoved += caretMoved;
        }
        return true;
    }

    /// <summary>Pinトグル後に追従させるエディタを返す。既存sourceがあるPin操作では固定を保つ。</summary>
    public EditorTab? ApplyPinState(bool isPinned, EditorTab? activeSource)
    {
        IsPinned = isPinned;
        return IsPinned && Source is not null ? null : activeSource;
    }
