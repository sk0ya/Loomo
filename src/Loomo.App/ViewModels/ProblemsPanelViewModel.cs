...
/// <summary>診断（Problems）一覧の1件。開いているエディタタブの <c>EffectiveDiagnostics</c>（LSPの
...
/// <summary>サイドバーの「診断（Problems）」パネル。開いている全エディタタブの診断を集約して一覧表示する。
/// 集約自体（LSPマネージャの購読・EffectiveDiagnosticsの読み出し）は <c>ShellWindow.Problems.cs</c>（View層。
/// VimEditorControl/Editor.Controls.Lsp に触れるのはそちら）が行い、ここは表示用データだけを持つ。</summary>
