...
/// <summary>
/// 隔離されたサブエージェントを 1 回走らせる実行器。<b>ツールの実装が内部で AI を活用する</b>ための基盤で、
/// メイン会話の履歴を運ばずに自己完結したサブタスク（要約・分類・整形・調査など）を実行し、<b>最終結果だけ</b>を返す。
/// エージェントに委譲ツールを提示して AI に委譲を選ばせるのではなく、ツール側が必要に応じてこれを呼ぶ。
