...
### 30.19.2 問題は、探しに行かないと見えなかった —— ガターの電球とスクロールバーの印

§30.19.1 で電球は押せるようになったが、電球は**ホバーしないと出てこない**。修正候補があることを
知るには、まず「この行に何かあるかもしれない」と思ってマウスを乗せるか、Alt+Enter を試し打ちする
必要がある——つまり**知っている人にしか使えない**入口のままだった。診断そのものも同じで、波線は
**画面に出ている範囲にしかない**。500 行のファイルの 400 行目のエラーは、スクロールして初めて存在が判る。

見せる場所を 2 つ足した（どちらも Editor 側。部屋は「どのファイルで出すか」だけを決める）。

- **ガターの電球列。** 置き場はガター最左のグリフ余白（blame の右＝これまでブレークポイントが居た場所）。
  ブレークポイントの方は専用列をやめて**行番号の上**へ移した（その行の数字は譲り、行番号のクリックで
  トグルする）ので、列は 1 本増えたのではなく**入れ替わった**——デバッグを有効にしてもガターは広がらない。
  キャレット行に修正候補があるときだけ電球が点り、押すとクイックフィックスの
  一覧が出る。候補は `HostCodeActionProvider`（C# は Roslyn）→ LSP の順で、Alt+Enter・ホバーの電球と
  **同じ経路**を通る。だから「押しても候補なし」の空振りは構造的に起きない。問い合わせは
  250ms の落ち着き待ち＋**診断のかかっている行だけ**に絞ってあり、キャレット移動のたびに Roslyn を
  叩くことはない。
- **スクロールバーの診断マーカー。** 全診断を行単位でスクロールバーへ射影する（1 行 1 印、
  同じ行なら重い方が勝つ）。**重大度では捨てない**——警告はもちろん、不要な using のような
  Hint も出す。本文では波線を引かず薄字にする種類だが、薄字は**見えている範囲にしか無い**ので、
  それだけだと「画面外に片付けどころがある」ことが判らない。うるささは色で分ける（エラー＝赤、
