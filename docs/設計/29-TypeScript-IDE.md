...
- **日常導線は「実行」タブ（npm スクリプト一覧＋出力）**：検出した package.json の scripts を
  左側の縦タブとして名前＋実体コマンド＋**種別バッジ**を一覧表示し、選択した対象を右側の出力で確認する。
  実行は**行の ▶**（選択行とホバー行にだけ出し、同じ位置の種別バッジと入れ替える＝常時の予約幅を作らない）
  ／Enter／ダブルクリック／出力側の実行ボタンから。出力の見出しは**直近に実行した対象**
  （`LastRunLabel`）で、選択中スクリプトではない——実行後に一覧で別の行を選んでも出力の名乗りが嘘にならない。
  scripts が空のときは理由（package.json が無い／scripts が無い）を一覧の中央に出す。
  出力の自動追従は**末尾を見ている間だけ**（遡って読んでいる最中に末尾へ引き戻さない）。
  フロント開発サーバー／Node／テストは
  デバッグへ、ビルド／整形／判定不能なスクリプトはデバッガを付けず実行タブの出力へ集める。
...
- `TsDebugView`：§28 `DebugView` のクローンで、トップレベルは「実行」（スクリプト一覧＋出力）・問題・デバッグ・テスト・構成に整理する。
  「デバッグ」はセッション中だけ表示し、その中へ変数・自動・コールスタック・スレッド・ブレークポイント・イミディエイトをまとめる。
  サブビュー
