...
- ネイティブ項目のうち**押しても目的を果たせない3本**を、Loomo 側で取捨した
  ...
  - 「この位置で使える修正」は**「Quick Fix」サブメニューへ差し替えた**。ネイティブ側は候補を
    ...
    `only: ["quickfix"]`。適用は `ApplyLspWorkspaceEdit` に集約されるので Undo も同じ道。適用前の編集プレビューダイアログは 2026-09-26 に撤去した——候補を選んだ時点で実行は決まっており、選んだ後に確認を挟むのは順序が逆）。
...
- サイドバーの C# ソリューションパネルを使えるものへ直した（別パネルへ移した直後の版は、IDE ペインに
  間借りしていた頃の見出しをそのまま持ち込んでいた）。効いたのは5点。
  - **単一TFMのTFM段を畳んだ**。プロジェクトを開いても `net10.0` が1つ挟まるだけで、ファイルへ届くのに
    2回開く必要があった。選択肢が1つしか無い段は選ばせる意味が無いので、TFM名も行に出さない
    （多TFMのときだけ従来どおり段として現れ、選択中は ● で示す）。
  - **名前の絞り込み**を足した。数千ファイルのツリーを目で追うのは無理で、開閉と縦スクロールしか
    移動手段が無かった。空白区切りはAND、一致した枝だけを開いた状態で出し、解除すると絞り込みのために
    開いた枝は元の（畳んだ）状態へ戻る。全開の暴発を防ぐため一致は300件で打ち切り、件数を出す。
    刈り込みは純関数 `CSharpSolutionTreeFilter`（`Loomo.CSharp`）で、表示側は結果を描くだけ。
  - **ビルド／テストの対象を「選んだ行の持ち主」へ遡らせた**。ファイルやフォルダーを選んだままボタンを
    押すと、対象がプロジェクトでないため**黙って何も起きなかった**（§23.3 に反する）。`RequestAction` が
    親をたどってプロジェクト（無ければソリューション）へ寄せ、その名前を操作行に出すので、押す前に
    何が走るか読める。テストを持たない対象ではテストボタンを無効化する。
  - **見出しを1行に詰め込むのをやめた**。ラベル＋構成＋🔨🧪＋実行状態が220pxに同居して実行状態が
    読めなかったので、見出し（`PanelHeader`・他のサイドバーパネルと同じ）＋構成と折りたたみ、対象と
    ビルド／テスト、絞り込み、状態、の各行へ分けた。状態行は空なら行ごと畳む（予約行を作らない）。
    アイコンだった🔨🧪は「ビルド」「テスト」の文字へ戻した——狭さは他の行を削って解決した。
  - **開閉を1クリックにした**（フォルダーツリーと同じ）。開閉矢印は7pxしかなく、`.csproj` やフォルダーの
    名前を押しても何も起きなかった。行のどこを押しても開閉し、矢印自身のクリックとダブルクリックは
    二重トグルになるので除外する。開閉はコンテナ（`TreeViewItem.IsExpanded`）へ書き、ノードVMとは
    TwoWayで結ばれているので利用者の開閉状態としても残る。ファイル行（子を持たない行）は従来どおり
    ダブルクリック／Enterで開く。
  - **右クリックがソリューションとプロジェクト以外で無反応だった**のを直した。ファイル・フォルダー・参照の
    行にも「開く」「パスをコピー」「エクスプローラーで表示」と、持ち主プロジェクトのビルド／テストを出す。
  - **行アイコンをフォルダーツリーと同じ絵にした**。記号（◈▣↗◆≡）では「その行が何なのか」が読めず、
    ファイルとフォルダーには記号すら無かったので、拡張子ごとのベクター線画（`FileIcons`／FolderTreeView と
    同じ共有インスタンス）へ置き換えた。`.sln` と `.csproj` は専用の絵を持ち、フォルダーとグループ見出し
    （参照・アナライザー・追加ファイル）は開閉で開いた絵／畳んだ絵が入れ替わる。記号が残るのは**多TFMの段だけ**
    （選択中＝●／それ以外＝○——ここだけ対応するファイルが無い）。開閉で絵を差し替えるためノードVMは
    `ObservableObject` になり（`IsExpanded` が変更通知を持つ）、テーマの明暗切替では `FileIcons.PaletteChanged`
    を購読してツリー全体のアイコンを引き直す（購読は `Dispose` で外す）。回帰は
    `CSharpSolutionExplorerViewTests` に1件（種類ごとの引き当て・フォルダーの開閉で絵と通知が変わる）。
  - **絞り込み中の「すべて折りたたむ」がツリーを2度作り直していた**のを直した。`FilterText = ""` の
    setter が既に `Rebuild()` を呼ぶのに末尾でもう一度呼んでいたため、1回の折りたたみで1万ノード級の
    ノードVMを2度捨てて作っていた（結果は同じなので目には見えないが、そのぶん固まる）。回帰は
    `CSharpSolutionTreeTests` に1件（`Nodes` へルートが差し込まれる回数＝作り直した回数）。
