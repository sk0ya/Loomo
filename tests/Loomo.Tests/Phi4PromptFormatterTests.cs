...
public class Phi4PromptFormatterTests
{
    ...
    public void Builds_system_with_tool_block_and_opens_assistant()
    {
        ...
        var prompt = Phi4PromptFormatter.Build(profile: null, workspaceFolders: System.Array.Empty<string>(), conv, Pwsh());
    ...
    public void Omits_tool_guidance_when_no_tools()
    {
        ...
        var prompt = Phi4PromptFormatter.Build(null, System.Array.Empty<string>(), conv, System.Array.Empty<ToolDefinition>());
    ...
    public void Includes_current_folder_in_system_prompt()
    {
        ...
        var prompt = Phi4PromptFormatter.Build(null, new[] { "C:\\proj" }, conv, System.Array.Empty<ToolDefinition>());
    ...
    public void Includes_all_workspace_folders_when_multi_root()
    {
        ...
        var prompt = Phi4PromptFormatter.Build(
            null, new[] { "C:\\proj", "C:\\other" }, conv, System.Array.Empty<ToolDefinition>());
    ...
    public void Renders_assistant_tool_calls_and_tool_results_as_turns()
    {
        ...
        var prompt = Phi4PromptFormatter.Build(null, System.Array.Empty<string>(), conv, Pwsh());
    ...
    public void Warmup_prefix_is_an_exact_prefix_of_the_first_real_turn()
    {
        ...
        // その system ブロックが、実ターン（同じ profile/root/tools ＋<|user|>…）の
        // 先頭と完全一致することが、KV プレフィックス再利用の前提。ここを回帰として固定する。
        var tools = Pwsh();
        var root = new[] { "C:\\proj" };

        var warmup = Phi4PromptFormatter.Build(AgentProfiles.Root, root, new Conversation(), tools);

        var conv = new Conversation();
        conv.AddUser("ファイル一覧を出して");
        var real = Phi4PromptFormatter.Build(AgentProfiles.Root, root, conv, tools);
    ...
    public void Render_prefix_is_injected_before_user_text_in_the_user_turn()
    {
        ...
        Assert.Contains("<|user|>" + SystemPrompts.WorkflowTurnPreamble + "\n\n次の文章を英語に翻訳してください。", prompt);
    }
    ...
    public void Render_prefix_lives_after_the_warmup_prefix_so_kv_sharing_holds()
    {
        ...
        Assert.StartsWith(systemBlock + "<|user|>" + SystemPrompts.ChatTurnPreamble, real);
    }

    [Fact]
    public void Default_system_prompt_is_engine_neutral_and_guides_tool_calling()
    {
        Assert.DoesNotContain("Ollama", SystemPrompts.Phi4);
        Assert.Contains("Tools only: run_powershell, write_file, edit_file, web_search.", SystemPrompts.Phi4);
        Assert.Contains("optionally wrapped in <|tool_call|>", SystemPrompts.Phi4);
        // run_powershell の呼び出し形を具体例で示し、小モデルが rg/read_file/build 等の架空ツール名へ崩れるのを抑える。
        Assert.Contains("\"name\":\"run_powershell\"", SystemPrompts.Phi4);
        Assert.Contains("\"arguments\":{\"command\":\"Get-ChildItem\"}", SystemPrompts.Phi4);
        Assert.Contains("Read file", SystemPrompts.Phi4);
        Assert.Contains("Search code", SystemPrompts.Phi4);
        Assert.Contains("dotnet build", SystemPrompts.Phi4);
        Assert.Contains("read_file/search/build are commands or files, not tools", SystemPrompts.Phi4);
        Assert.Contains("Use tools for workspace facts/actions", SystemPrompts.Phi4);
        Assert.Contains("answer chat directly", SystemPrompts.Phi4);
        Assert.Contains("non-interactive", SystemPrompts.Phi4);
        // 構造化ファイルツールがプロンプトに告知されていること。
        Assert.Contains("write_file", SystemPrompts.Phi4);
        Assert.Contains("\"name\":\"edit_file\"", SystemPrompts.Phi4);
        Assert.Contains("\"name\":\"web_search\"", SystemPrompts.Phi4);
        Assert.Contains("Read before editing existing files", SystemPrompts.Phi4);
        Assert.Contains("no Set-Content", SystemPrompts.Phi4);
    }
}
